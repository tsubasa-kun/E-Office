...
public class RoundImageView extends ImageView {
    ...
    private void setCustomAttributes(AttributeSet attrs) {
        ...
        mBorderInsideThickness = a.getDimensionPixelSize(R.styleable.roundedimageview_border_inside_thickness, 0);
        mBorderOutsideThickness = a.getDimensionPixelSize(R.styleable.roundedimageview_border_outside_thickness, 0);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBorderInsideColor != defaultColor && mBorderOutsideColor != defaultColor) {// 定义画两个边框，分别为外圆边框和内圆边框
            radius = (defaultWidth < defaultHeight ? defaultWidth : defaultHeight) / 2 - mBorderOutsideThickness - mBorderInsideThickness;
            // 画外圆
            drawCircleBorder(canvas, radius + mBorderOutsideThickness / 2, mBorderOutsideColor);
            // 画内圆
            drawCircleBorder(canvas, radius + mBorderInsideThickness / 2, mBorderInsideColor);
        } else if (mBorderInsideColor != defaultColor && mBorderOutsideColor == defaultColor) {// 定义画一个边框
            radius = (defaultWidth < defaultHeight ? defaultWidth : defaultHeight) / 2 - mBorderInsideThickness;
            drawCircleBorder(canvas, radius + mBorderInsideThickness / 2, mBorderInsideColor);
        } else if (mBorderInsideColor == defaultColor && mBorderOutsideColor != defaultColor) {// 定义画一个边框
            radius = (defaultWidth < defaultHeight ? defaultWidth : defaultHeight) / 2 - mBorderOutsideThickness;
            drawCircleBorder(canvas, radius + mBorderOutsideThickness / 2, mBorderOutsideColor);
        } else {// 没有边框
